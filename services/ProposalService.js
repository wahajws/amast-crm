  ...
  async findAll(options = {}, currentUser = null) {
    try {
      ...
      if (currentUser) {
        ...
        // VIEWER and USER roles can only see their own proposals
        if (userRole === 'VIEWER' || userRole === 'USER') {
          if (!options.where) {
            options.where = 'p.owner_id = ?';
            options.params = [currentUser.userId];
          } else {
            options.where = `(${options.where}) AND p.owner_id = ?`;
            options.params = [...(options.params || []), currentUser.userId];
          }
        }
        // MANAGER can see team proposals
        ...
        // SUPER_ADMIN and ADMIN can see all proposals
      }
  ...
  async update(id, data, user = null) {
    try {
      ...
      if (user) {
        ...
        if (userRole === 'VIEWER') {
          throw new Error('Viewers cannot update proposals');
        }

        // USER and MANAGER can only update their own proposals
  ...
  async delete(id, user = null) {
    try {
      ...
      if (user) {
        ...
        if (userRole === 'VIEWER') {
          throw new Error('Viewers cannot delete proposals');
        }

        // USER and MANAGER can only delete their own proposals
        if ((userRole === 'USER' || userRole === 'MANAGER') && existing.ownerId !== user.userId) {
